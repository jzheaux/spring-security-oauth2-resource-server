...
class RemoteKeySetSecurityConfig extends WebSecurityConfigurerAdapter {
	@Autowired
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		String jwks = this.server.url("/.well-known/jwks.json").toString();

		// @formatter:off
		http
			.apply(new ResourceServerConfigurer<>())
				.jwt()
					.signature().keys(url(jwks).connectTimeout(30000L));
		// @formatter:on
	}
}
